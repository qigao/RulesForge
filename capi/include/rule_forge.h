...
// --- Symbol Visibility ---
  ...
  #ifdef SHARED_CXX
    #define RULEFORGE_API __declspec(dllexport)
  #else
    #define RULEFORGE_API __declspec(dllimport)
  #endif
#elif defined(__GNUC__) && __GNUC__ >= 4
  #define RULEFORGE_API __attribute__((visibility("default")))
#else
  #define RULEFORGE_API
#endif
...
RULEFORGE_API ruleforge_status_t ruleforge_init(void);
RULEFORGE_API ruleforge_status_t ruleforge_cleanup(void);

// Returns the library version string (e.g., "0.1.0").
RULEFORGE_API const char *ruleforge_get_version(void);

RULEFORGE_API const char *ruleforge_get_last_error_message(void);
...
RULEFORGE_API ruleforge_status_t ruleforge_kb_load_drl(ruleforge_knowledge_base_t kb,
...
// 30}"). Returns RULES_FORGE_OK on success.
...
// Returns RULES_FORGE_OK on success.
RULEFORGE_API ruleforge_status_t ruleforge_session_fire_all_rules(ruleforge_stateful_session_t session,
...
RULEFORGE_API ruleforge_status_t ruleforge_fact_get_field_as_string(ruleforge_fact_t fact,
...
// Returns RULES_FORGE_OK on success.
RULEFORGE_API ruleforge_status_t ruleforge_fact_get_field_as_double(ruleforge_fact_t fact,
                                                                const char *field_name,
                                                                double *out_value);

// Gets a fact field as an integer (64-bit).
// Returns RULES_FORGE_OK on success.
RULEFORGE_API ruleforge_status_t ruleforge_fact_get_field_as_int(ruleforge_fact_t fact,
                                                             const char *field_name,
                                                             int64_t *out_value);

// Gets a fact field as a boolean.
// Returns RULES_FORGE_OK on success.
RULEFORGE_API ruleforge_status_t ruleforge_fact_get_field_as_bool(
