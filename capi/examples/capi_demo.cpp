...
int main() {
  std::cout << "RuleForge CAPI Demo - Version " << ruleforge_get_version() << std::endl;
...
query "AdultPersonsQuery"
  ...
  if (ruleforge_session_add_fact_json(session, "Person", R"({"name": "Alice", "age": 25})") !=
      DRILLS_OK) {
    print_error_and_exit("Failed to add fact Alice.");
  }
  if (ruleforge_session_add_fact_json(session, "Person", R"({"name": "Bob", "age": 17})") !=
      DRILLS_OK) {
    print_error_and_exit("Failed to add fact Bob.");
  }
  if (ruleforge_session_add_fact_json(session, "Person", R"({"name": "Charlie", "age": 30})") !=
      DRILLS_OK) {
    print_error_and_exit("Failed to add fact Charlie.");
  }
  std::cout << "Fact count: " << ruleforge_session_get_fact_count(session) << std::endl;
  ...
  std::cout << "Rules fired: " << fired_count << std::endl;
  ...
  for (int i = 0; i < result_count; ++i) {
    ...
    if (ruleforge_fact_get_field_as_string(person_fact, "name", name_buffer, sizeof(name_buffer),
                                           &actual_length) != DRILLS_OK) {
