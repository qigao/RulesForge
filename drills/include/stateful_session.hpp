...
public:
    ...
    explicit StatefulSession(private_key, std::shared_ptr<KnowledgeBase const> kb);
    ...
    QueryResult execute_query(std::string const& query_name, std::vector<std::shared_ptr<Fact>> const& args = {});
    ...
    void add_fact_typed(T const& typed_fact) {
        std::shared_ptr<Fact> fact = kb_->get_fact_type_registry().convert(typed_fact);
        if (fact) { add_fact(fact); }
    }
    ...
    std::vector<std::shared_ptr<IEngineListener>>& get_listeners() { return listeners_; }
    ...
    inline std::shared_ptr<TokenWME const> get_or_create_wme(std::shared_ptr<TokenWME const> parent_wme,
        ...
        if (!parent_wme) parent_wme = dummy_wme_;
        size_t fact_hash_key = (fact->id != 0) ? static_cast<size_t>(fact->id) : reinterpret_cast<size_t>(fact.get());
        size_t parent_hash = parent_wme ? parent_wme->hash : 0;
        size_t hash = parent_hash ^ (fact_hash_key + 0x9e3779b9 + (parent_hash << 6) + (parent_hash >> 2));
        auto new_wme = std::make_shared<TokenWME>(TokenWME{parent_wme, fact, parent_wme->depth + 1, hash});
        auto it = wme_cache_.find(new_wme);
        if (it != wme_cache_.end()) { return *it; }
...
private:
    ...
    std::vector<std::shared_ptr<ReteNode>> build_alpha_chain(ConstraintNode const* node,
                                                             std::vector<std::shared_ptr<ReteNode>> parent_tails);

    using TokenWMESet = std::unordered_set<std::shared_ptr<TokenWME const>, TokenWMEPtrHasher, TokenWMEPtrEquals>;
    ...
    std::map<std::string, std::shared_ptr<ReteNode>> alpha_entry_points_;
    std::map<std::string, std::shared_ptr<QueryTerminalNode>> query_nodes_;
    std::map<std::string, std::shared_ptr<QueryInputNode>> parameterized_query_inputs_;

    // Working memory state
    std::shared_ptr<TokenWME const> dummy_wme_;
    TokenWMESet wme_cache_;
    std::map<int64_t, std::shared_ptr<Fact>> all_facts_;
    std::vector<std::string> agenda_group_focus_stack_;
    std::map<size_t, Activation> agenda_map_;
