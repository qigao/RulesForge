...
namespace {
    ...
    void discover_and_merge_files(std::string const& file_path, DroolsParserContext& context) {
        try {
                ...
                if (is_wildcard) {
                    if (std::filesystem::is_directory(full_path)) {
                        ...
                        for (auto const& dir_entry : std::filesystem::directory_iterator(full_path)) {
                            if (dir_entry.is_regular_file() && dir_entry.path().extension() == ".drl") {
                    ...
                    full_path.replace_extension(".drl");
...
std::shared_ptr<KnowledgeBase> build_knowledge_base(std::string const& file_path,
    ...
    DroolsParserContext context{.base_dirs = base_dirs, .result = result};

    // PASS 1: PARSE & MERGE
    // This part recursively parses all files and merges them into a single context.state.
    // The AstBuilder stamps each rule/query with its source_package and source_imports.
    ...
    SemanticAnalyzer analyzer(context.state, file_path);

    // PASS 2: BUILD SCHEMA
    // This runs first on the complete, merged state. It populates the analyzer's
    // internal schema with ALL declared types from ALL files.
    ...
    // Now, analyze the rules and queries. The analyzer already has the complete schema,
    // and it can use the context stamped on each rule to resolve short names correctly.
