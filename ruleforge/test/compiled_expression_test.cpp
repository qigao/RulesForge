...
suite("CompiledExpression") {

    group("Basic arithmetic") {
        it("evaluates simple addition") {
            auto expr = CompiledExpression::compile("1 + 2");
            check(expr != nullptr);
        ...
        it("evaluates all basic operators") {
            auto expr = CompiledExpression::compile("10 + 5 - 3 * 2 / 2");
            check(expr != nullptr);
            double result = expr->evaluate([](std::string const&) { return 0.0; });
        ...
        it("respects parentheses") {
            auto expr = CompiledExpression::compile("(10 + 5) * 2");
            check(expr != nullptr);
            double result = expr->evaluate([](std::string const&) { return 0.0; });
    ...
    group("Variables") {
        ...
        it("handles multiple variables") {
            auto expr = CompiledExpression::compile("$price - $discount");
            check(expr != nullptr);
        ...
        it("handles dotted variable names") {
            auto expr = CompiledExpression::compile("$order.total * $tax.rate");
            check(expr != nullptr);
            check(expr->variables().size() == 2);
        ...
        it("handles same variable used multiple times") {
            auto expr = CompiledExpression::compile("$x + $x * $x");
            check(expr != nullptr);
            check(expr->variables().size() == 1);
    ...
    group("Math functions") {
        it("evaluates sin and cos") {
            auto expr = CompiledExpression::compile("sin(0) + cos(0)");
            check(expr != nullptr);
            double result = expr->evaluate([](std::string const&) { return 0.0; });
            // sin(0) = 0, cos(0) = 1
        ...
        it("evaluates sqrt") {
            auto expr = CompiledExpression::compile("sqrt($x)");
            check(expr != nullptr);
            double result = expr->evaluate([](std::string const& var) {
        ...
        it("evaluates exp and log") {
            auto expr = CompiledExpression::compile("log(exp(2))");
            check(expr != nullptr);
            double result = expr->evaluate([](std::string const&) { return 0.0; });
        ...
        it("evaluates pow / exponentiation") {
            auto expr = CompiledExpression::compile("$base ^ $exp");
            check(expr != nullptr);
            double result = expr->evaluate([](std::string const& var) {
        ...
        it("evaluates abs") {
            auto expr = CompiledExpression::compile("abs($x)");
            check(expr != nullptr);
            double result = expr->evaluate([](std::string const& var) {
        ...
        it("evaluates min and max") {
            auto expr = CompiledExpression::compile("min($a, $b) + max($a, $b)");
            check(expr != nullptr);
            double result = expr->evaluate([](std::string const& var) {
                if (var == "$a") return 3.0;
                if (var == "$b") return 7.0;
        ...
        it("evaluates floor and ceil") {
            auto expr = CompiledExpression::compile("floor(3.7) + ceil(3.2)");
            check(expr != nullptr);
            double result = expr->evaluate([](std::string const&) { return 0.0; });
        ...
        it("evaluates clamp") {
            auto expr = CompiledExpression::compile("clamp(-1, $x, 1)");
            check(expr != nullptr);
    ...
    group("Constants") {
        it("provides pi constant") {
            auto expr = CompiledExpression::compile("pi");
            check(expr != nullptr);
            double result = expr->evaluate([](std::string const&) { return 0.0; });
        ...
        it("provides infinity constant") {
            auto expr = CompiledExpression::compile("inf");
            check(expr != nullptr);
            double result = expr->evaluate([](std::string const&) { return 0.0; });
        ...
        it("can compute e using exp(1)") {
            auto expr = CompiledExpression::compile("exp(1)");
            check(expr != nullptr);
            double result = expr->evaluate([](std::string const&) { return 0.0; });
    ...
    group("Complex expressions") {
        it("evaluates Pythagorean theorem") {
            auto expr = CompiledExpression::compile("sqrt($a^2 + $b^2)");
            check(expr != nullptr);
            double result = expr->evaluate([](std::string const& var) {
                if (var == "$a") return 3.0;
                if (var == "$b") return 4.0;
        ...
        it("evaluates compound interest formula") {
            // A = P * (1 + r)^t
            auto expr = CompiledExpression::compile("$principal * (1 + $rate)^$years");
            check(expr != nullptr);
            double result = expr->evaluate([](std::string const& var) {
        ...
        it("verifies trigonometric identity sin^2 + cos^2 = 1") {
            auto expr = CompiledExpression::compile("sin($x)^2 + cos($x)^2");
            check(expr != nullptr);
            double result = expr->evaluate([](std::string const& var) {
    ...
    group("Error handling") {
        it("rejects empty expression") {
            std::string error;
            auto expr = CompiledExpression::compile("", &error);
            check(expr == nullptr);
            check(!error.empty());
        }
        ...
        it("rejects invalid syntax") {
            std::string error;
            auto expr = CompiledExpression::compile("1 + * 2", &error);
            check(expr == nullptr);
            check(!error.empty());
        }
        ...
        it("rejects unknown function") {
            std::string error;
            auto expr = CompiledExpression::compile("unknownfunc(1)", &error);
            check(expr == nullptr);
            check(!error.empty());
        }
    }
    ...
    group("Accessors") {
        it("returns original expression string") {
            ...
            auto expr = CompiledExpression::compile(original);
            check(expr != nullptr);
