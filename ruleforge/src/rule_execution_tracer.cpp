...
void RuleExecutionTracer::trace_rule_fired(std::string const& rule_name, 
    ...
    RuleTraceEvent event(RuleTraceEvent::Type::RULE_FIRED, rule_name, 
...
void RuleExecutionTracer::trace_fact_added(int64_t fact_id, std::string const& fact_type) {
    ...
    RuleTraceEvent event(RuleTraceEvent::Type::FACT_ADDED, "", 
                        "Added fact of type " + fact_type);
...
std::vector<RuleTraceEvent> RuleExecutionTracer::get_rule_events(std::string const& rule_name) const {
...
std::vector<RuleExecutionTracer::RuleStats> RuleExecutionTracer::get_rule_statistics() const {
    ...
    for (auto const& event : trace_events_) {
        if (event.rule_name.empty()) continue;
...
std::string RuleExecutionTracer::format_rule_summary() const {
    ...
    oss << std::left << std::setw(30) << "Rule Name" 
        << std::setw(8) << "Fired" 
        << std::setw(10) << "Matched"
    ...
    for (auto const& stat : stats) {
        oss << std::left << std::setw(30) << stat.rule_name
            << std::setw(8) << stat.fire_count
            << std::setw(10) << stat.match_count
            << std::setw(12) << stat.total_execution_time_us
