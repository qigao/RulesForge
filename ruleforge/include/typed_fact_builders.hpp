...
public:
    ...
    std::shared_ptr<Fact> build() const& {
        return std::make_shared<Fact>(*fact_);
    }

private:
    friend class FactBuilderBase<PersonBuilder>;
    
    PersonBuilder() : fact_(std::make_shared<Fact>()) {
        fact_->type = "Person";
    }
...
private:
    ...
    CustomerBuilder() : fact_(std::make_shared<Fact>()) {
        fact_->type = "Customer";
    }
...
private:
    ...
    OrderBuilder() : fact_(std::make_shared<Fact>()) {
        fact_->type = "Order";
    }
