...
struct js_traits<std::variant<Ts...>>
{
  ...
  static std::optional<std::variant<Ts...>> unwrapObj(JSContext* ctx,
                                                      JSValueConst v,
                                                      JSClassID class_id)
  {
  ...
  static std::variant<Ts...> unwrap(JSContext* ctx, JSValueConst v)
  {
    ...
    switch (tag) {
      ...
      case JS_TAG_BOOL:
        return unwrapPriority<is_boolean,
                              std::is_integral,
...
Tuple unwrap_args_impl(JSContext* ctx,
                       int argc,
                       JSValueConst* argv,
 ...
 */
template<typename... Args>
std::tuple<std::decay_t<Args>...> unwrap_args(JSContext* ctx,
                                              int argc,
                                              JSValueConst* argv)
{
...
JSValue wrap_call(JSContext* ctx,
                  Callable&& f,
                  int argc,
...
JSValue wrap_this_call(JSContext* ctx,
                       Callable&& f,
...
template<class Tuple, std::size_t... I>
void wrap_args_impl(JSContext* ctx,
                    JSValue* argv,
                    Tuple tuple,
                    std::index_sequence<I...>)
{
                                   ...
                                   ptr_cast_fcn_t ptr_cast_fcn)
  {
            ...
            derived_ptr_cast_fcn_t derived_ptr_cast_fcn)
    {
      ...
      registerDerivedClass<D>(
          derived_class_id,
          [derived_cast_fcn = std::move(derived_ptr_cast_fcn)](JSContext* ctx,
                                                               JSValueConst v)
...
{
  static void set_property(JSContext* ctx,
                           JSValue this_obj,
                           Key key,
                           JSValue value);
...
struct js_property_traits<const char*>
{
  ...
  static JSValue get_property(JSContext* ctx,
                              JSValue this_obj,
...
public:
  ...
  Value evalThis(std::string_view buffer,
          ...
          [](JSContext* ctx, JSModuleDef* m) noexcept
          {
            ...
            for (const auto& e : it->exports) {
              if (JS_SetModuleExport(
                      ctx, m, e.first, JS_DupValue(ctx, e.second.v))
                  != 0)
              {
                return -1;
              }
...
public:
  ...
  /// @see JS_Eval
  Value eval(std::string_view buffer,
  ...
  /// @see JS_ParseJSON2
  Value fromJSON(std::string_view buffer,
  ...
  static auto unwrap(JSContext* ctx, JSValueConst fun_obj)
  {
    ...
    if constexpr (argc == 0) {
                  ...
                  Value {ctx, JS_DupValue(ctx, fun_obj)}](Args... args) -> R
      {
        ...
        JSValue result = JS_Call(jsfun_obj.ctx,
                                 jsfun_obj.v,
                                 JS_UNDEFINED,
                                 argc,
