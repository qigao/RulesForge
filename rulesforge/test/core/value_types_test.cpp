...
suite("ConstraintValue") {
    group("bool") {
        it("stores booleans without collapsing them into integers") {
            ...
            check_equal(enabled_text.c_str(), "true");
            check_equal(disabled_text.c_str(), "false");
        }
    ...
    group("DataBind scalar identities") {
        it("keeps each extended scalar as a distinct runtime type") {
            ...
            check_equal(unique.size(), values.size());
            ...
            check_equal(enum_text.c_str(), "Sell");
            check_equal(datetime_text.c_str(), "2026-06-28T09:30:05.123Z");
            check_equal(date_text.c_str(), "2026-06-28");
            check_equal(time_text.c_str(), "09:30:05.123");
            check_equal(decimal_text.c_str(), "123.45");
            check_equal(money_text.c_str(), "USD 123.45");
        }
        ...
        it("validates extended scalars against exact schema field types") {
            ...
            check_equal(validator.validate(fact).size(), 1);
        }
    }
}
