...
suite("Agenda Benchmarks") {
    bench("Original Agenda") {
        benchmark("add 10k operations", 1) {
            Agenda agenda;
        ...
        benchmark("pop 10k operations", 1) {
            Agenda agenda;
            for (size_t i = 0; i < 10000; ++i) {
                int salience = static_cast<int>(i % 2000) - 1000;
                agenda.add(create_mock_activation(i, salience));
    ...
    bench("Priority Queues") {
        benchmark("BucketQueue push 10k", 1) {
        ...
        benchmark("BucketQueue pop 10k", 1) {
            BucketPriorityQueue queue;
            for (size_t i = 0; i < 10000; ++i) {
                Priority p = static_cast<Priority>(i % 4);
                queue.push(p, i);
        ...
        benchmark("BucketQueue batch pop", 1) {
            BucketPriorityQueue queue;
            for (size_t i = 0; i < 10000; ++i) {
                Priority p = static_cast<Priority>(i % 4);
                queue.push(p, i);
