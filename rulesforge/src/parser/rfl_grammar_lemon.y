...
%type jmespath_input { ParsedJmesPath* }
%destructor jmespath_input { delete $$; }

// from jmespath("json-string", "expression")
// from jmespath(file("path/to.json"), "expression")
from_clause(A) ::= FROM JMESPATH LPAREN jmespath_input(S) COMMA STRING(E) RPAREN. {
    S->engine_kind = ParsedJmesPath::EngineKind::JmesPath;
    S->expression = unescape_string_literal(strip_quotes(E.as_sv()));
    A = new PatternSource(std::move(*S));
    delete S;
}

// from dsv("csv-string", "filter-expression")
// from dsv(file("path/to.csv"), "filter-expression")
from_clause(A) ::= FROM DSV LPAREN jmespath_input(S) COMMA STRING(E) RPAREN. {
    S->engine_kind = ParsedJmesPath::EngineKind::DsvFilter;
    S->expression = unescape_string_literal(strip_quotes(E.as_sv()));
    A = new PatternSource(std::move(*S));
    delete S;
}

from_clause(A) ::= FROM CSV LPAREN jmespath_input(S) COMMA STRING(E) RPAREN. {
    S->engine_kind = ParsedJmesPath::EngineKind::DsvFilter;
    S->expression = unescape_string_literal(strip_quotes(E.as_sv()));
    A = new PatternSource(std::move(*S));
    delete S;
...
jmespath_input(A) ::= STRING(S). {
    A = new ParsedJmesPath();
    A->input_kind = ParsedJmesPath::InputKind::JsonString;
    A->input_value = unescape_string_literal(strip_quotes(S.as_sv()));
}

jmespath_input(A) ::= IDENTIFIER(FN) LPAREN STRING(S) RPAREN. {
    auto fn = FN.as_string();
    if (fn == "file") {
        A = new ParsedJmesPath();
        A->input_kind = ParsedJmesPath::InputKind::JsonFile;
        A->input_value = unescape_string_literal(strip_quotes(S.as_sv()));
    } else {
        ctx->add_error(FN.line, FN.column,
                       "Unsupported jmespath source function '" + fn + "'. Expected file(\"...\").");
        A = new ParsedJmesPath();
        A->input_kind = ParsedJmesPath::InputKind::JsonString;
        A->input_value.clear();
    }
...
accum_source_clause(A) ::= FROM JMESPATH LPAREN jmespath_input(S) COMMA STRING(E) RPAREN. {
    S->engine_kind = ParsedJmesPath::EngineKind::JmesPath;
    S->expression = unescape_string_literal(strip_quotes(E.as_sv()));
    A = new PatternSource(std::move(*S));
    delete S;
...
accum_source_clause(A) ::= FROM DSV LPAREN jmespath_input(S) COMMA STRING(E) RPAREN. {
    S->engine_kind = ParsedJmesPath::EngineKind::DsvFilter;
    S->expression = unescape_string_literal(strip_quotes(E.as_sv()));
    A = new PatternSource(std::move(*S));
    delete S;
...
accum_source_clause(A) ::= FROM CSV LPAREN jmespath_input(S) COMMA STRING(E) RPAREN. {
    S->engine_kind = ParsedJmesPath::EngineKind::DsvFilter;
    S->expression = unescape_string_literal(strip_quotes(E.as_sv()));
    A = new PatternSource(std::move(*S));
    delete S;
