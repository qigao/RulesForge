                                       ...
                                       int depth)
{
  ...
  std::visit(
      [&](auto&& arg)
      {
        ...
        if constexpr (std::is_same_v<T, ParsedJmesPath>) {
          logd("  -> Analyzing '{}' source: kind={}, value='{}', expr='{}'",
               arg.engine_kind == ParsedJmesPath::EngineKind::DsvFilter ? "dsv" : "jmespath",
               arg.input_kind == ParsedJmesPath::InputKind::JsonFile ? "file" : "string",
               arg.input_value, arg.expression);
          if (arg.input_value.empty()) {
            add_error(pattern_pos,
                      "In rule '" + rule.name
                          + "', jmespath source input cannot be empty.");
          }
          if (arg.expression.empty()) {
            add_error(pattern_pos,
                      "In rule '" + rule.name
                          + "', jmespath source has empty expression.");
          }
        }
      },
      pattern.source);
  ...
  if (pattern.type == PatternType::EVAL && pattern.eval_expression.has_value())
  {
    ...
    // Keep eval expression in DSL form ($var references stay unchanged).
...
void SemanticAnalyzer::analyze_rhs(ParsedRule& rule, SymbolTable const& symbols)
{
  ...
  logd("Analyzing RHS of rule '{}' with native RHS parser", rule.name);

  // Build bindings map from symbols
  std::map<std::string, int> bindings;
  for (auto const& [name, info] : symbols) {
    if (!name.empty() && name[0] == '$') {
      bindings[name] = info.depth;
    }
  }
  ...
  // Resolve type names in INSERT actions to fully qualified names
  for (auto& action : rule.compiled_actions) {
    if (action.type == RhsActionType::INSERT || action.type == RhsActionType::INSERT_LOGICAL) {
      if (auto resolved = resolve_type(action.target_type, rule.source_package, rule.source_imports)) {
        action.target_type = *resolved;
      } else {
        add_error(rule.pos, "In rule '" + rule.name + "', INSERT uses undeclared type '" + action.target_type + "'");
        return;
      }
    }
  }

  logd("Native RHS analysis completed successfully for rule '{}' with {} actions",
       rule.name, rule.compiled_actions.size());
}
