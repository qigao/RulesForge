...
RhsParser::Token RhsParser::Lexer::scan_identifier() {
    ...
    if (text == "update") type = Token::TOK_UPDATE;
    else if (text == "insert") type = Token::TOK_INSERT;
    else if (text == "insertLogical") type = Token::TOK_INSERT_LOGICAL;
    else if (text == "retract") type = Token::TOK_RETRACT;
    else if (text == "halt") type = Token::TOK_HALT;
    else if (text == "setFocus") type = Token::TOK_SET_FOCUS;
    else if (text == "invoke") type = Token::TOK_INVOKE;
    else if (text == "if") type = Token::TOK_IF;
    else if (text == "else") type = Token::TOK_ELSE;
    else if (text == "for") type = Token::TOK_FOR;
    else if (text == "in") type = Token::TOK_IN;
    else if (text == "while") type = Token::TOK_WHILE;
    else if (text == "switch") type = Token::TOK_SWITCH;
    else if (text == "case") type = Token::TOK_CASE;
    else if (text == "default") type = Token::TOK_DEFAULT;
    else if (text == "break") type = Token::TOK_BREAK;
    else if (text == "continue") type = Token::TOK_CONTINUE;
    else if (text == "true") type = Token::TOK_TRUE;
    else if (text == "false") type = Token::TOK_FALSE;
...
RhsParser::Token RhsParser::Lexer::next() {
    ...
    if (pos_ + 1 < input_.size()) {
        ...
        if (two == "==") { pos_ += 2; column_ += 2; return {Token::TOK_EQ, two, line_, start_col}; }
        if (two == "!=") { pos_ += 2; column_ += 2; return {Token::TOK_NE, two, line_, start_col}; }
        if (two == ">=") { pos_ += 2; column_ += 2; return {Token::TOK_GE, two, line_, start_col}; }
        if (two == "<=") { pos_ += 2; column_ += 2; return {Token::TOK_LE, two, line_, start_col}; }
        if (two == "&&") { pos_ += 2; column_ += 2; return {Token::TOK_AND, two, line_, start_col}; }
        if (two == "||") { pos_ += 2; column_ += 2; return {Token::TOK_OR, two, line_, start_col}; }
    }
    ...
    column_++;
    switch (c) {
        case '{': return {Token::TOK_LBRACE, "{", line_, start_col};
        case '}': return {Token::TOK_RBRACE, "}", line_, start_col};
        case '(': return {Token::TOK_LPAREN, "(", line_, start_col};
        case ')': return {Token::TOK_RPAREN, ")", line_, start_col};
        case ',': return {Token::TOK_COMMA, ",", line_, start_col};
        case '.': return {Token::TOK_DOT, ".", line_, start_col};
        case '=': return {Token::TOK_ASSIGN, "=", line_, start_col};
        case '+': return {Token::TOK_PLUS, "+", line_, start_col};
        case '-': return {Token::TOK_MINUS, "-", line_, start_col};
        case '*': return {Token::TOK_STAR, "*", line_, start_col};
        case '/': return {Token::TOK_SLASH, "/", line_, start_col};
        case '%': return {Token::TOK_MOD, "%", line_, start_col};
        case '^': return {Token::TOK_CARET, "^", line_, start_col};
        case '?': return {Token::TOK_QUESTION, "?", line_, start_col};
        case ':': return {Token::TOK_COLON, ":", line_, start_col};
        case '>': return {Token::TOK_GT, ">", line_, start_col};
        case '<': return {Token::TOK_LT, "<", line_, start_col};
        default: return {Token::TOK_ERROR, std::string(1, c), line_, start_col};
    }
}
...
bool RhsParser::Parser::check(Token::Type type) {
...
CompiledAction RhsParser::Parser::parse_action() {
    switch (current_.type) {
        case Token::TOK_UPDATE: return parse_update();
        case Token::TOK_INSERT: return parse_insert();
        case Token::TOK_INSERT_LOGICAL: return parse_insert_logical();
        case Token::TOK_RETRACT: return parse_retract();
        case Token::TOK_HALT: return parse_halt();
        case Token::TOK_SET_FOCUS: return parse_set_focus();
        case Token::TOK_INVOKE: return parse_invoke();
        case Token::TOK_IF: return parse_if();
        case Token::TOK_FOR: return parse_for();
        case Token::TOK_WHILE: return parse_while();
        case Token::TOK_SWITCH: return parse_switch();
        case Token::TOK_BREAK: return parse_break();
        case Token::TOK_CONTINUE: return parse_continue();
        default:
            error("Expected action keyword (update, insert, retract, halt, setFocus, invoke, if, for, while, switch, break, continue)");
            return {};
    }
}
...
CompiledAction RhsParser::Parser::parse_invoke() {
    ...
    if (!check(Token::TOK_RPAREN)) {
        ...
        while (match(Token::TOK_COMMA)) {
            if (check(Token::TOK_RPAREN)) break;
            action.invoke_args.push_back(parse_value());
        }
    }
...
std::vector<FieldAssignment> RhsParser::Parser::parse_field_assignments() {
    ...
    while (match(Token::TOK_COMMA)) {
        if (check(Token::TOK_RBRACE)) break;  // Trailing comma
        assignments.push_back(parse_field_assignment());
    }
...
std::string RhsParser::Parser::parse_comparison() {
    ...
    while (check(Token::TOK_GT) || check(Token::TOK_LT) || check(Token::TOK_GE) || check(Token::TOK_LE) ||
           check(Token::TOK_EQ) || check(Token::TOK_NE)) {
        Token op = advance();
        expr += " " + op.text + " ";
        expr += parse_additive();
    }
...
std::string RhsParser::Parser::parse_term() {
    ...
    if (check(Token::TOK_LPAREN)) {
        ...
        if (check(Token::TOK_LPAREN)) {
            ...
            term = name;
            // Handle field access: field.subfield
            while (check(Token::TOK_DOT)) {
                advance();
                Token field = consume(Token::TOK_IDENTIFIER, "Expected field name after '.'");
                term += "." + field.text;
