#ifndef __RULES_FORGE_H__
#define __RULES_FORGE_H__

#include "rules_forge_export.h"
...
RULES_FORGE_C_API ruleforge_status_t ruleforge_init(void);
RULES_FORGE_C_API ruleforge_status_t ruleforge_cleanup(void);

// Returns the library version string (e.g., "0.9.0").
RULES_FORGE_C_API const char *ruleforge_get_version(void);
...
// same thread.
RULES_FORGE_C_API const char *ruleforge_get_last_error_message(void);
...
//   }
RULES_FORGE_C_API ruleforge_status_t ruleforge_data_bind_object_from_binary(
    const char *schema_path, const char *fact_type, const uint8_t *data,
    size_t len, ruleforge_data_bind_object_t *out_object);
RULES_FORGE_C_API ruleforge_status_t ruleforge_data_bind_object_from_json(
    const char *schema_path, const char *fact_type, const char *json,
    size_t len, ruleforge_data_bind_object_t *out_object);
RULES_FORGE_C_API ruleforge_status_t ruleforge_data_bind_object_from_yaml(
    const char *schema_path, const char *fact_type, const char *yaml,
    size_t len, ruleforge_data_bind_object_t *out_object);
RULES_FORGE_C_API ruleforge_status_t ruleforge_data_bind_object_from_xml(
...
RULES_FORGE_C_API ruleforge_status_t ruleforge_data_bind_object_from_csv(
    const char *schema_path, const char *fact_type, const char *csv,
    size_t len, size_t row, ruleforge_data_bind_object_t *out_object);
RULES_FORGE_C_API ruleforge_status_t ruleforge_data_bind_object_clone(
...
RULES_FORGE_C_API const char *ruleforge_data_bind_object_get_type_name(
    ruleforge_data_bind_object_t object);

// Serialized text buffers are owned by the caller and must be released with
// ruleforge_data_bind_serialized_free(). Binary buffers use
// ruleforge_data_bind_binary_free(). out_len is optional for text formats; output
// buffers are set to NULL on failure. Writers return RULES_FORGE_ERROR_GENERIC when the sink
// rejects a chunk and otherwise use the same status mapping as serialization.
RULES_FORGE_C_API ruleforge_status_t ruleforge_data_bind_object_serialize_json(
    ruleforge_data_bind_object_t object, char **out_json, size_t *out_len);
RULES_FORGE_C_API ruleforge_status_t ruleforge_data_bind_object_serialize_yaml(
    ruleforge_data_bind_object_t object, char **out_yaml, size_t *out_len);
RULES_FORGE_C_API ruleforge_status_t ruleforge_data_bind_object_serialize_xml(
    ruleforge_data_bind_object_t object, char **out_xml, size_t *out_len);
RULES_FORGE_C_API ruleforge_status_t ruleforge_data_bind_object_serialize_csv(
    ruleforge_data_bind_object_t object, char **out_csv, size_t *out_len);
RULES_FORGE_C_API ruleforge_status_t ruleforge_data_bind_object_serialize_binary(
    ruleforge_data_bind_object_t object, uint8_t **out_binary, size_t *out_len);
RULES_FORGE_C_API ruleforge_status_t ruleforge_data_bind_object_write_json(
    ruleforge_data_bind_object_t object, ruleforge_write_fn write, void *user);
RULES_FORGE_C_API ruleforge_status_t ruleforge_data_bind_object_write_yaml(
    ruleforge_data_bind_object_t object, ruleforge_write_fn write, void *user);
RULES_FORGE_C_API ruleforge_status_t ruleforge_data_bind_object_write_xml(
    ruleforge_data_bind_object_t object, ruleforge_write_fn write, void *user);
RULES_FORGE_C_API ruleforge_status_t ruleforge_data_bind_object_write_csv(
    ruleforge_data_bind_object_t object, ruleforge_write_fn write, void *user);
RULES_FORGE_C_API void ruleforge_data_bind_serialized_free(char *data);
RULES_FORGE_C_API void ruleforge_data_bind_binary_free(void *data);
RULES_FORGE_C_API ruleforge_status_t ruleforge_data_bind_object_destroy(
    ruleforge_data_bind_object_t object);
...
RULES_FORGE_C_API ruleforge_status_t ruleforge_kb_create(ruleforge_knowledge_base_t *out_kb);

// Loads RFL rules into the Knowledge Base from an in-memory source string.
// Returns RULES_FORGE_OK on success.
RULES_FORGE_C_API ruleforge_status_t ruleforge_kb_load_drl(ruleforge_knowledge_base_t kb,
...
// Returns RULES_FORGE_OK on success.
RULES_FORGE_C_API ruleforge_status_t ruleforge_kb_load_drl_file(ruleforge_knowledge_base_t kb,
...
// base_dir_count: Number of entries in base_dirs.
// Returns RULES_FORGE_OK on success.
RULES_FORGE_C_API ruleforge_status_t ruleforge_kb_load_drl_files(ruleforge_knowledge_base_t kb,
...
// Returns RULES_FORGE_OK on success.
RULES_FORGE_C_API ruleforge_status_t ruleforge_kb_load_decision_table_csv(ruleforge_knowledge_base_t kb,
                                                                  const char *csv_source);

// Destroys a Knowledge Base and frees its associated resources.
// The handle becomes invalid after this call.
RULES_FORGE_C_API ruleforge_status_t ruleforge_kb_destroy(ruleforge_knowledge_base_t kb);
RULES_FORGE_C_API ruleforge_status_t ruleforge_kb_has_schema_type(
    ruleforge_knowledge_base_t kb, const char *type_name, int *out_exists);
...
RULES_FORGE_C_API ruleforge_status_t ruleforge_session_create(ruleforge_knowledge_base_t kb,
...
RULES_FORGE_C_API ruleforge_status_t ruleforge_continuous_push_data_bind_object(
    ruleforge_continuous_session_t session,
...
RULES_FORGE_C_API ruleforge_status_t ruleforge_continuous_push_json(
    ruleforge_continuous_session_t session, const char *fact_type,
    const char *event_id, const char *entry_point,
    int64_t event_time_ms, const char *fact_json,
    ruleforge_continuous_result_t *out_result);

// Parses one schema-bound YAML mapping and commits it as one event step.
RULES_FORGE_C_API ruleforge_status_t ruleforge_continuous_push_yaml(
    ruleforge_continuous_session_t session, const char *fact_type,
    const char *event_id, const char *entry_point,
...
RULES_FORGE_C_API ruleforge_status_t ruleforge_continuous_push_json_path(
    ruleforge_continuous_session_t session, const char *fact_type,
    const char *json_source, const char *json_path,
    const char *event_id_field, const char *event_time_field,
    const char *entry_point, ruleforge_continuous_result_t *out_result);
RULES_FORGE_C_API ruleforge_status_t ruleforge_continuous_push_yaml_path(
    ruleforge_continuous_session_t session, const char *fact_type,
    const char *yaml_source, const char *yaml_path,
    const char *event_id_field, const char *event_time_field,
    const char *entry_point, ruleforge_continuous_result_t *out_result);
RULES_FORGE_C_API ruleforge_status_t ruleforge_continuous_push_csv_path(
    ruleforge_continuous_session_t session, const char *fact_type,
    const char *csv_source, const char *csv_path,
    const char *event_id_field, const char *event_time_field,
    const char *entry_point, ruleforge_continuous_result_t *out_result);
RULES_FORGE_C_API ruleforge_status_t ruleforge_continuous_push_xml_path(
    ruleforge_continuous_session_t session, const char *fact_type,
    const char *xml_source, const char *xml_path,
    const char *event_id_field, const char *event_time_field,
    const char *entry_point, ruleforge_continuous_result_t *out_result);

RULES_FORGE_C_API ruleforge_status_t ruleforge_continuous_advance_watermark(
    ruleforge_continuous_session_t session, int64_t watermark_ms,
    ruleforge_continuous_result_t *out_result);
RULES_FORGE_C_API ruleforge_status_t ruleforge_continuous_drain(
    ruleforge_continuous_session_t session,
    ruleforge_continuous_result_t *out_result);
RULES_FORGE_C_API ruleforge_status_t ruleforge_continuous_acknowledge(
    ruleforge_continuous_session_t session, uint64_t batch_id);
RULES_FORGE_C_API ruleforge_status_t ruleforge_continuous_get_metrics(
    ruleforge_continuous_session_t session,
...
RULES_FORGE_C_API ruleforge_status_t ruleforge_continuous_data_bind_stream_json_create(
    ruleforge_continuous_session_t session, const char *fact_type,
    const char *event_id, const char *entry_point,
    int64_t event_time_ms, ruleforge_continuous_data_bind_stream_t *out_stream);
RULES_FORGE_C_API ruleforge_status_t ruleforge_continuous_data_bind_stream_yaml_create(
    ruleforge_continuous_session_t session, const char *fact_type,
    const char *event_id, const char *entry_point,
    int64_t event_time_ms, ruleforge_continuous_data_bind_stream_t *out_stream);
...
RULES_FORGE_C_API ruleforge_status_t ruleforge_continuous_data_bind_stream_json_path_create(
    ruleforge_continuous_session_t session, const char *fact_type,
    const char *json_path, const char *event_id_field,
    const char *event_time_field, const char *entry_point,
    ruleforge_continuous_data_bind_stream_t *out_stream);
RULES_FORGE_C_API ruleforge_status_t ruleforge_continuous_data_bind_stream_yaml_path_create(
    ruleforge_continuous_session_t session, const char *fact_type,
    const char *yaml_path, const char *event_id_field,
    const char *event_time_field, const char *entry_point,
    ruleforge_continuous_data_bind_stream_t *out_stream);
RULES_FORGE_C_API ruleforge_status_t ruleforge_continuous_data_bind_stream_csv_path_create(
    ruleforge_continuous_session_t session, const char *fact_type,
    const char *csv_path, const char *event_id_field,
    const char *event_time_field, const char *entry_point,
    ruleforge_continuous_data_bind_stream_t *out_stream);
RULES_FORGE_C_API ruleforge_status_t ruleforge_continuous_data_bind_stream_xml_path_create(
    ruleforge_continuous_session_t session, const char *fact_type,
    const char *xml_path, const char *event_id_field,
    const char *event_time_field, const char *entry_point,
    ruleforge_continuous_data_bind_stream_t *out_stream);
RULES_FORGE_C_API ruleforge_status_t ruleforge_continuous_data_bind_stream_feed(
    ruleforge_continuous_data_bind_stream_t stream, const void *data, size_t len);
RULES_FORGE_C_API ruleforge_status_t ruleforge_continuous_data_bind_stream_feed_file(
    ruleforge_continuous_data_bind_stream_t stream, const char *file_path);
RULES_FORGE_C_API ruleforge_status_t ruleforge_continuous_data_bind_stream_finish(
    ruleforge_continuous_data_bind_stream_t stream,
    ruleforge_continuous_result_t *out_result);
RULES_FORGE_C_API ruleforge_status_t ruleforge_continuous_data_bind_stream_destroy(
    ruleforge_continuous_data_bind_stream_t stream);
...
RULES_FORGE_C_API ruleforge_status_t ruleforge_continuous_result_get_output(
    ruleforge_continuous_result_t result, int index, ruleforge_fact_t *out_fact);
RULES_FORGE_C_API ruleforge_status_t ruleforge_continuous_result_destroy(
...
RULES_FORGE_C_API ruleforge_status_t ruleforge_session_add_data_bind_object(
...
RULES_FORGE_C_API ruleforge_status_t ruleforge_session_add_data_bind_value(
...
RULES_FORGE_C_API ruleforge_status_t ruleforge_session_add_fact_json(
    ruleforge_stateful_session_t session, const char *fact_type,
    const char *fact_json, ruleforge_fact_t *out_fact);
...
RULES_FORGE_C_API ruleforge_status_t ruleforge_data_bind_stream_json_create(
    ruleforge_stateful_session_t session, const char *fact_type,
    ruleforge_data_bind_stream_t *out_stream);
RULES_FORGE_C_API ruleforge_status_t ruleforge_data_bind_stream_json_all_create(
    ruleforge_stateful_session_t session, const char *fact_type,
    ruleforge_data_bind_stream_t *out_stream);
RULES_FORGE_C_API ruleforge_status_t ruleforge_data_bind_stream_json_path_create(
    ruleforge_stateful_session_t session, const char *fact_type, const char *json_path,
    ruleforge_data_bind_stream_t *out_stream);
RULES_FORGE_C_API ruleforge_status_t ruleforge_data_bind_stream_json_path_all_create(
    ruleforge_stateful_session_t session, const char *fact_type, const char *json_path,
    ruleforge_data_bind_stream_t *out_stream);
RULES_FORGE_C_API ruleforge_status_t ruleforge_data_bind_stream_yaml_create(
    ruleforge_stateful_session_t session, const char *fact_type,
    ruleforge_data_bind_stream_t *out_stream);
RULES_FORGE_C_API ruleforge_status_t ruleforge_data_bind_stream_yaml_all_create(
    ruleforge_stateful_session_t session, const char *fact_type,
    ruleforge_data_bind_stream_t *out_stream);
RULES_FORGE_C_API ruleforge_status_t ruleforge_data_bind_stream_yaml_path_create(
    ruleforge_stateful_session_t session, const char *fact_type, const char *yaml_path,
    ruleforge_data_bind_stream_t *out_stream);
RULES_FORGE_C_API ruleforge_status_t ruleforge_data_bind_stream_yaml_path_all_create(
    ruleforge_stateful_session_t session, const char *fact_type, const char *yaml_path,
    ruleforge_data_bind_stream_t *out_stream);
RULES_FORGE_C_API ruleforge_status_t ruleforge_data_bind_stream_csv_all_create(
    ruleforge_stateful_session_t session, const char *fact_type,
    ruleforge_data_bind_stream_t *out_stream);
RULES_FORGE_C_API ruleforge_status_t ruleforge_data_bind_stream_csv_path_create(
    ruleforge_stateful_session_t session, const char *fact_type, const char *csv_path,
    ruleforge_data_bind_stream_t *out_stream);
RULES_FORGE_C_API ruleforge_status_t ruleforge_data_bind_stream_xml_create(
    ruleforge_stateful_session_t session, const char *fact_type,
    ruleforge_data_bind_stream_t *out_stream);
RULES_FORGE_C_API ruleforge_status_t ruleforge_data_bind_stream_xml_path_all_create(
    ruleforge_stateful_session_t session, const char *fact_type, const char *xml_path,
    ruleforge_data_bind_stream_t *out_stream);
...
RULES_FORGE_C_API ruleforge_status_t ruleforge_data_bind_stream_feed(
    ruleforge_data_bind_stream_t stream, const void *data, size_t len);
RULES_FORGE_C_API ruleforge_status_t ruleforge_data_bind_stream_feed_file(
...
RULES_FORGE_C_API ruleforge_status_t ruleforge_data_bind_stream_finish(
    ruleforge_data_bind_stream_t stream, ruleforge_fact_t **out_facts,
    int *out_loaded_count);
RULES_FORGE_C_API ruleforge_status_t ruleforge_data_bind_stream_destroy(
    ruleforge_data_bind_stream_t stream);

// Frees an array returned by schema-bound multi-fact input APIs.
RULES_FORGE_C_API void ruleforge_fact_array_free(ruleforge_fact_t *facts);
...
// Returns RULES_FORGE_OK on success.
RULES_FORGE_C_API ruleforge_status_t ruleforge_session_fire_all_rules(ruleforge_stateful_session_t session,
...
// Returns RULES_FORGE_OK on success.
RULES_FORGE_C_API ruleforge_status_t ruleforge_session_reset(ruleforge_stateful_session_t session);

// Gets the number of facts currently in the session's working memory.
// Returns the fact count, or -1 on error.
RULES_FORGE_C_API int ruleforge_session_get_fact_count(ruleforge_stateful_session_t session);
...
RULES_FORGE_C_API ruleforge_status_t ruleforge_session_query(ruleforge_stateful_session_t session,
                                                     const char *query_name,
                                                     ruleforge_query_result_t *out_query_result);

// Destroys a Stateful Session and frees its associated resources.
// The handle becomes invalid after this call.
RULES_FORGE_C_API ruleforge_status_t ruleforge_session_destroy(ruleforge_stateful_session_t session);
...
RULES_FORGE_C_API ruleforge_status_t ruleforge_session_enable_tracing(ruleforge_stateful_session_t session,
                                                              int enabled);
...
RULES_FORGE_C_API ruleforge_status_t ruleforge_session_clear_trace(ruleforge_stateful_session_t session);
...
RULES_FORGE_C_API int64_t ruleforge_session_get_memory_used(ruleforge_stateful_session_t session);

// Gets the peak memory usage of the session's arena allocator (in bytes).
// Returns the peak memory, or -1 on error.
RULES_FORGE_C_API int64_t ruleforge_session_get_memory_peak(ruleforge_stateful_session_t session);
...
RULES_FORGE_C_API int ruleforge_query_result_get_size(ruleforge_query_result_t query_result);
...
// The handle becomes invalid after this call.
RULES_FORGE_C_API ruleforge_status_t ruleforge_query_result_destroy(ruleforge_query_result_t query_result);
...
RULES_FORGE_C_API ruleforge_status_t ruleforge_fact_get_field_as_string(ruleforge_fact_t fact,
...
// Returns RULES_FORGE_OK on success.
RULES_FORGE_C_API ruleforge_status_t ruleforge_fact_get_field_as_double(ruleforge_fact_t fact,
                                                                const char *field_name,
                                                                double *out_value);

// Gets a fact field as an integer (64-bit).
// Returns RULES_FORGE_OK on success.
RULES_FORGE_C_API ruleforge_status_t ruleforge_fact_get_field_as_int(ruleforge_fact_t fact,
                                                             const char *field_name,
                                                             int64_t *out_value);

// Gets an exact unsigned 64-bit field. Signed integers are not coerced.
RULES_FORGE_C_API ruleforge_status_t ruleforge_fact_get_field_as_uint64(
...
RULES_FORGE_C_API ruleforge_status_t ruleforge_fact_get_field_as_bytes(
    ruleforge_fact_t fact, const char *field_name, uint8_t *buffer,
    size_t buffer_size, size_t *out_actual_length);

// Gets a fact field as a boolean.
// Returns RULES_FORGE_OK on success.
RULES_FORGE_C_API ruleforge_status_t ruleforge_fact_get_field_as_bool(
    ruleforge_fact_t fact, const char *field_name, int *out_value);

RULES_FORGE_C_API ruleforge_status_t ruleforge_fact_get_field_as_enum(
    ruleforge_fact_t fact, const char *field_name, char *name_buffer,
    size_t buffer_size, size_t *out_actual_length, int64_t *out_value);
RULES_FORGE_C_API ruleforge_status_t ruleforge_fact_get_enum_name(
    ruleforge_fact_t fact, const char *field_name, char *name_buffer,
    size_t buffer_size, size_t *out_actual_length);
