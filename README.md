...
- **SaltsUtils** for concrete parser and utility components.
...
DataBind is resolved independently through `DATABIND_ROOT`; SaltsUtils remains a separate parser/utility dependency. The physical DataBind repository extraction may still be staged, but package ownership is no longer delegated to SaltsUtils.
...
The top-level CMake configuration resolves the three ownership roots explicitly: Salts from `SALTS_ROOT`, parser/utilities from `SALTS_UTILS_ROOT`, and DataBind from `DATABIND_ROOT`. No DataBind fallback through SaltsUtils is accepted.
