...
void SemanticAnalyzer::build_schema()
{
  ...
  // Register enum types (enums have no fields, just symbolic values)
  for (auto& enum_decl : state_.parsed_enums) {
    if (!enum_decl.source_package.empty()
        && enum_decl.enum_name.find('.') == std::string::npos)
    {
      enum_decl.enum_name = enum_decl.source_package + "." + enum_decl.enum_name;
    }
    logd("  -> Enum '{}': {} values", enum_decl.enum_name, enum_decl.values.size());
    type_schemas_[enum_decl.enum_name] = {};  // Empty field set for enums
  }
}
