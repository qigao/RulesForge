...
FieldType tbe_type_to_field_type(std::string_view type)
{
  if (type == "string") {
    return FT_String;
  }
...
{
  FieldType field_type = tbe_type_to_field_type(type_name);
                          ...
                          std::map<std::string, std::string> const& enum_underlying_types)
{
  ...
  if (field.is_enum) {
    ...
    if (enum_it != enum_underlying_types.end()) {
      out.type = tbe_type_to_field_type(enum_it->second);
      return out;
    }
  }

  out.type = tbe_type_to_field_type(type);
...
std::string enum_underlying_type(std::string const& tbe_underlying_type)
{
  FieldType field_type = tbe_type_to_field_type(tbe_underlying_type);
  switch (field_type) {
    case FT_Long:
      return "long";
    case FT_Int:
      return "int";
    default:
      return tbe_underlying_type.empty() ? "int" : tbe_underlying_type;
  }
}
                        ...
                        SchemaImport const& import)
{
  ...
  if (status != DATA_BIND_OK) {
    errors.push_back({.file_name = schema_path.string(),
                      ...
                      .message = "TurboScript DataBind schema load failed: "
...
bool import_turboscript_schemas(parser_state& state,
                                std::vector<std::string> const& base_dirs,
                                std::vector<StructuredError>& errors,
                                std::string const& default_source_name)
{
  ...
  for (auto const& import : state.schema_imports) {
    ...
    if (!resolved) {
      ...
      errors.push_back({.file_name = source_name,
                        .line = import.line,
                        .column = import.column,
                        .message = "Could not resolve TurboScript schema import: " + import.path});
      ok = false;
      continue;
    }
