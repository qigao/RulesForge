...
static char* msort(char*, char**, int (*)(char const*, char const*));
...
static int lemon_vsprintf(char* str, char const* zFormat, va_list ap) {
    int i, j, k, c;
    int nUsed = 0;
    char const* z;
    char zTemp[50];
...
static int lemon_sprintf(char* str, char const* format, ...) {
    va_list ap;
    int rc;
    va_start(ap, format);
...
static void lemon_strcpy(char* dest, char const* src) {
    while ((*(dest++) = *(src++)) != 0) {}
}
static void lemon_strcat(char* dest, char const* src) {
    while (*dest) dest++;
...
void ErrorMsg(char const*, int, char const*, ...);

/****** From the file "option.h" ******************************************/
enum option_type { OPT_FLAG = 1, OPT_INT, OPT_DBL, OPT_STR, OPT_FFLAG, OPT_FINT, OPT_FDBL, OPT_FSTR };
struct s_options {
...
struct rule {
    ...
    char const* lhsalias;   /* Alias for the LHS (NULL if none) */
    int lhsStart;           /* True if left-hand side is the start symbol */
    int ruleline;           /* Line number for the rule */
    int nrhs;               /* Number of RHS symbols */
    struct symbol** rhs;    /* The RHS symbols */
    char const** rhsalias;  /* An alias for each RHS symbol (NULL if none) */
    int line;               /* Line number at which code begins */
    char const* code;       /* The code executed when this rule is reduced */
    char const* codePrefix; /* Setup code before code[] above */
    char const* codeSuffix; /* Breakdown code after code[] above */
...
char const* Strsafe(char const*);

void Strsafe_init(void);
int Strsafe_insert(char const*);
char const* Strsafe_find(char const*);

/* Routines for handling symbols of the grammar */

struct symbol* Symbol_new(char const*);
int Symbolcmpp(void const*, void const*);
void Symbol_init(void);
int Symbol_insert(struct symbol*, char const*);
...
int Configcmp(char const*, char const*);
...
static int actioncmp(struct action* ap1, struct action* ap2) {
    ...
    if (rc == 0 && (ap1->type == REDUCE || ap1->type == SHIFTREDUCE)) { rc = ap1->x.rp->index - ap2->x.rp->index; }
    if (rc == 0) { rc = (int)(ap2 - ap1); }
    return rc;
}

/* Sort parser actions */
static struct action* Action_sort(struct action* ap) {
    ap = (struct action*)msort((char*)ap, (char**)&ap->next, (int (*)(char const*, char const*))actioncmp);
    return ap;
}
...
void acttab_action(acttab* p, int lookahead, int action) {
    if (p->nLookahead >= p->nLookaheadAlloc) {
        ...
        p->aLookahead =
            (struct lookahead_action*)lemon_realloc(p->aLookahead, sizeof(p->aLookahead[0]) * p->nLookaheadAlloc);
        if (p->aLookahead == 0) {
...
int acttab_insert(acttab* p, int makeItSafe) {
    ...
    */
    ...
    if (p->nAction + n >= p->nActionAlloc) {
        ...
        p->aAction = (struct lookahead_action*)lemon_realloc(p->aAction, sizeof(p->aAction[0]) * p->nActionAlloc);
        if (p->aAction == 0) {
            fprintf(stderr, "malloc failed\n");
            exit(1);
        }
    ...
    for (i = p->nAction - 1; i >= end; i--) {
        if (p->aAction[i].lookahead == p->mnLookahead) {
            ...
            ** transaction is allowed to match aAction[i] */
            ...
            if (n == p->nLookahead) { break; /* An exact match is found at offset i */ }
        }
    }
        ...
        ** transaction will be appended. */
        ...
        for (; i < p->nActionAlloc - p->mxLookahead; i++) {
            if (p->aAction[i].lookahead < 0) {
                ...
                if (j == p->nAction) { break; /* Fits in empty slots */ }
            }
        }
    }
...
void FindFirstSets(struct lemon* lemp) {
    ...
    for (i = 0; i < lemp->nsymbol; i++) { lemp->symbols[i]->lambda = LEMON_FALSE; }
    for (i = lemp->nterminal; i < lemp->nsymbol; i++) { lemp->symbols[i]->firstset = SetNew(); }
        ...
        for (rp = lemp->rule; rp; rp = rp->next) {
            ...
            for (i = 0; i < rp->nrhs; i++) {
                ...
                if (s2->type == TERMINAL) {
                    ...
                    for (j = 0; j < s2->nsubsym; j++) { progress += SetAdd(s1->firstset, s2->subsym[j]->index); }
                    break;
                } else if (s1 == s2) {
    ...
    ** state.  Initially, all configurations are incomplete */
    ...
    for (cfp = stp->cfp; cfp; cfp = cfp->next) {
        ...
        ** on the symbol "sp" */
        if (sp->type == MULTITERMINAL) {
            int i;
            for (i = 0; i < sp->nsubsym; i++) { Action_add(&stp->ap, SHIFT, sp->subsym[i], (char*)newstp); }
        } else {
...
void FindLinks(struct lemon* lemp) {
    ...
    ** which the link is attached. */
    for (i = 0; i < lemp->nstate; i++) {
        stp = lemp->sorted[i];
        for (cfp = stp ? stp->cfp : 0; cfp; cfp = cfp->next) { cfp->stp = stp; }
    }
...
void FindFollowSets(struct lemon* lemp) {
    ...
    for (i = 0; i < lemp->nstate; i++) {
        ...
        for (cfp = lemp->sorted[i]->cfp; cfp; cfp = cfp->next) { cfp->status = INCOMPLETE; }
    }
...
static int resolve_conflict(struct action* apx, struct action* apy) {
    ...
    if (apx->type == SHIFT && apy->type == REDUCE) {
        ...
        assert(apx->type == SH_RESOLVED || apx->type == RD_RESOLVED || apx->type == SSCONFLICT ||
               apx->type == SRCONFLICT || apx->type == RRCONFLICT || apy->type == SH_RESOLVED ||
               apy->type == RD_RESOLVED || apy->type == SSCONFLICT || apy->type == SRCONFLICT ||
               apy->type == RRCONFLICT);
...
PRIVATE struct config* newconfig(void) { return (struct config*)lemon_calloc(1, sizeof(struct config)); }
...
struct config* Configlist_add(struct rule* rp, /* The rule */
            ...
            for (newrp = sp->rule; newrp; newrp = newrp->nextlhs) {
                ...
                for (i = dot + 1; i < rp->nrhs; i++) {
                    ...
                    if (xsp->type == TERMINAL) {
                        ...
                        int k;
                        for (k = 0; k < xsp->nsubsym; k++) { SetAdd(newcfp->fws, xsp->subsym[k]->index); }
                        break;
                    } else {
...
void ErrorMsg(char const* filename, int lineno, char const* format, ...) {
    va_list ap;
...
static void handle_T_option(char* z) {
    ...
    if (user_templatename == 0) { memory_error(); }
...
static struct rule* Rule_sort(struct rule* rp) {
    ...
    rp = 0;
    for (i = 0; i < sizeof(x) / sizeof(x[0]); i++) { rp = Rule_merge(x[i], rp); }
    return rp;
}

/* forward reference */
static char const* minimum_size_type(int lwr, int upr, int* pnByte);

/* Print a single line of the "Parser Stats" output
 */
static void stats_line(char const* zLabel, int iValue) {
...
*/
static int defineCmp(void const* pA, void const* pB) {
    char const* zA = *(char const**)pA;
    char const* zB = *(char const**)pB;
    return strcmp(zA, zB);
}
...
int main(int argc, char** argv) {
    ...
    static struct s_options options[] = {
        ...
        {OPT_FLAG, "S", (char*)&sqlFlag, "Generate the *.sql file describing the parser tables."},
    ...
    */
    for (i = 0, rp = lem.rule; rp; rp = rp->next) { rp->iRule = rp->code ? i++ : -1; }
    ...
    if (lem.nconflict > 0) { fprintf(stderr, "%d parsing conflicts.\n", lem.nconflict); }
...
**   changed.
*/
static char* merge(char* a, char* b, int (*cmp)(char const*, char const*), int offset) {
    char *ptr, *head;
...
static int handleflags(int i, FILE* err) {
    ...
    if (op[j].label == 0) {
        ...
        errcnt++;
    } else if (op[j].arg == 0) { /* Ignore this option */
...
void OptPrint(void) {
    ...
    for (i = 0; op[i].label; i++) {
        switch (op[i].type) {
            ...
            case OPT_INT:
            case OPT_FINT:
                fprintf(errstream, "  -%s<integer>%*s  %s\n", op[i].label, (int)(max - lemonStrlen(op[i].label) - 9),
                        "", op[i].message);
                break;
            case OPT_DBL:
            case OPT_FDBL:
                fprintf(errstream, "  -%s<real>%*s  %s\n", op[i].label, (int)(max - lemonStrlen(op[i].label) - 6), "",
                        op[i].message);
                break;
            case OPT_STR:
            case OPT_FSTR:
                fprintf(errstream, "  -%s<string>%*s  %s\n", op[i].label, (int)(max - lemonStrlen(op[i].label) - 8), "",
                        op[i].message);
                break;
        }
    }
...
struct pstate {
    ...
    char const* lhsalias;       /* Alias for the LHS */
    int nrhs;                   /* Number of right-hand side symbols seen */
    struct symbol* rhs[MAXRHS]; /* RHS symbols */
    char const* alias[MAXRHS];  /* Aliases for each RHS symbol (or NULL) */
    struct rule* prevrule;      /* Previous rule parsed */
    char const* declkeyword;    /* Keyword of a declaration */
    ...
    switch (psp->state) {
        ...
        case WAITING_FOR_DECL_OR_RULE:
            if (x[0] == '%') {
                ...
                ErrorMsg(psp->filename, psp->tokenlineno, "Token \"%s\" should be either \"%%\" or a nonterminal name.",
                         x);
                psp->errorcnt++;
            }
            break;
        ...
        case LHS_ALIAS_2:
            if (x[0] == ')') {
                ...
                ErrorMsg(psp->filename, psp->tokenlineno, "Missing \")\" following LHS alias name \"%s\".",
                         psp->lhsalias);
                psp->errorcnt++;
                psp->state = RESYNC_AFTER_RULE_ERROR;
            }
        ...
        case LHS_ALIAS_3:
            if (x[0] == ':' && x[1] == ':' && x[2] == '=') {
                ...
                ErrorMsg(psp->filename, psp->tokenlineno, "Missing \"->\" following: \"%s(%s)\".", psp->lhs->name,
                         psp->lhsalias);
                psp->errorcnt++;
                psp->state = RESYNC_AFTER_RULE_ERROR;
            }
        ...
        case IN_RHS:
            if (x[0] == '.') {
                struct rule* rp;
                rp = (struct rule*)lemon_calloc(
                    sizeof(struct rule) + sizeof(struct symbol*) * psp->nrhs + sizeof(char*) * psp->nrhs, 1);
                if (rp == 0) {
                    ...
                    rp->rhsalias = (char const**)&(rp->rhs[psp->nrhs]);
                ...
                if (psp->nrhs >= MAXRHS) {
                    ErrorMsg(psp->filename, psp->tokenlineno, "Too many symbols on RHS of rule beginning at \"%s\".",
                             x);
                    psp->errorcnt++;
                    psp->state = RESYNC_AFTER_RULE_ERROR;
                } else {
                ...
                msp->nsubsym++;
                msp->subsym = (struct symbol**)lemon_realloc(msp->subsym, sizeof(struct symbol*) * msp->nsubsym);
        ...
        case WAITING_FOR_DECL_ARG:
            if (x[0] == '{' || x[0] == '\"' || ISALNUM(x[0])) {
                ...
                n = nOld + nNew + 20;
                addLineMacro = !psp->gp->nolinenosflag && psp->insertLineMacro && psp->tokenlineno > 1 &&
                ...
                *psp->declargslot = (char*)lemon_realloc(*psp->declargslot, n);
                ...
                if (psp->decllinenoslot && psp->decllinenoslot[0] == 0) { psp->decllinenoslot[0] = psp->tokenlineno; }
...
static void preprocess_input(char* z) {
    ...
    for (i = 0; z[i]; i++) {
        ...
        if (strncmp(&z[i], "%endif", 6) == 0 && ISSPACE(z[i + 6])) {
            ...
            for (j = i; z[j] && z[j] != '\n'; j++) z[j] = ' ';
        } else if (strncmp(&z[i], "%ifdef ", 7) == 0 || strncmp(&z[i], "%if ", 4) == 0 ||
 ...
 */
PRIVATE char* file_makename(struct lemon* lemp, char const* suffix) {
    char* name;
    char* cp;
...
** to the stream */
PRIVATE FILE* file_open(struct lemon* lemp, char const* suffix, char const* mode) {
    FILE* fp;
...
void rule_print(FILE* out, struct rule* rp) {
    ...
    for (i = 0; i < rp->nrhs; i++) {
        ...
        if (sp->type == MULTITERMINAL) {
            ...
            for (j = 1; j < sp->nsubsym; j++) { fprintf(out, "|%s", sp->subsym[j]->name); }
        } else {
...
void RulePrint(FILE* fp, struct rule* rp, int iCursor) {
    ...
    for (i = 0; i <= rp->nrhs; i++) {
        ...
        if (sp->type == MULTITERMINAL) {
            ...
            for (j = 1; j < sp->nsubsym; j++) { fprintf(fp, "|%s", sp->subsym[j]->name); }
        } else {
 ...
 */
void ConfigPrint(FILE* fp, struct config* cfp) { RulePrint(fp, cfp->rp, cfp->dot); }
...
int PrintAction(struct action* ap, /* The action to print */
    ...
    switch (ap->type) {
        ...
        case SH_RESOLVED:
            if (showPrecedenceConflict) {
                fprintf(fp, "%*s shift        %-7d -- dropped by precedence", indent, ap->sp->name,
                        ap->x.stp->statenum);
            } else {
                result = 0;
            }
    ...
    if (result && ap->spOpt) { fprintf(fp, "  /* because %s==%s */", ap->sp->name, ap->spOpt->name); }
    return result;
}
...
void ReportOutput(struct lemon* lemp) {
    ...
    for (i = 0; i < lemp->nsymbol; i++) {
        ...
        if (sp->type == NONTERMINAL) {
            fprintf(fp, ":");
            if (sp->lambda) { fprintf(fp, " <lambda>"); }
            for (j = 0; j < lemp->nterminal; j++) {
                if (sp->firstset && SetFind(sp->firstset, j)) { fprintf(fp, " %s", lemp->symbols[j]->name); }
            }
        }
    ...
    for (rp = lemp->rule; rp; rp = rp->next) {
        ...
        rule_print(fp, rp);
        fprintf(fp, ".");
        if (rp->precsym) { fprintf(fp, " [%s precedence=%d]", rp->precsym->name, rp->precsym->prec); }
        fprintf(fp, "\n");
    }
    fclose(fp);
    return;
...
PRIVATE void tplt_skip_header(FILE* in, int* lineno) {
    char line[LINESIZE];
    while (fgets(line, LINESIZE, in) && (line[0] != '%' || line[1] != '%')) { (*lineno)++; }
}
...
*/
PRIVATE char* append_str(char const* zText, int n, int p1, int p2) {
...
PRIVATE int translate_code(struct lemon* lemp, struct rule* rp) {
    ...
    char const* zSkip = 0; /* The zOvwrt comment within rp->code, or NULL */
            ...
            if (rp->lhsalias && strcmp(cp, rp->lhsalias) == 0) {
                ...
                for (i = 0; i < rp->nrhs; i++) {
                    if (rp->rhsalias[i] && strcmp(cp, rp->rhsalias[i]) == 0) {
                        if (i == 0 && dontUseRhs0) {
                            ErrorMsg(lemp->filename, rp->ruleline, "Label %s used after '%s'.", rp->rhsalias[0],
                                     zOvwrt);
                            lemp->errorcnt++;
    ...
    if (rp->lhsalias && !lhsused) {
        ErrorMsg(lemp->filename, rp->ruleline, "Label \"%s\" for \"%s(%s)\" is never used.", rp->lhsalias,
...
void print_stack_union(FILE* out,          /* The output stream */
    ...
    char const* name; /* Name of the parser */
    ...
    maxdtlength = 0;
    if (lemp->vartype) { maxdtlength = lemonStrlen(lemp->vartype); }
    for (i = 0; i < lemp->nsymbol; i++) {
        int len;
        struct symbol* sp = lemp->symbols[i];
    ...
    */
    for (i = 0; i < lemp->nsymbol; i++) {
        ...
        hash = 0;
        for (j = 0; stddt[j]; j++) { hash = hash * 53 + stddt[j]; }
...
static void writeRuleText(FILE* out, struct rule* rp) {
    ...
    for (j = 0; j < rp->nrhs; j++) {
        ...
        if (sp->type != MULTITERMINAL) {
            ...
            int k;
            fprintf(out, " %s", sp->subsym[0]->name);
            for (k = 1; k < sp->nsubsym; k++) { fprintf(out, "|%s", sp->subsym[k]->name); }
        }
    }
}

/* Generate C source code for the parser */
void ReportTable(struct lemon* lemp, int mhflag, /* Output in makeheaders format if true */
    ...
    if (sqlFlag == 0) {
        ...
        for (i = 0; i < lemp->nsymbol; i++) {
            fprintf(sql,
                    ...
                    "VALUES(%d,'%s',%s",
                    i, lemp->symbols[i]->name, i < lemp->nterminal ? "TRUE" : "FALSE");
        ...
        for (i = 0, rp = lemp->rule; rp; rp = rp->next, i++) {
            assert(i == rp->iRule);
            fprintf(sql, "INSERT INTO rule(ruleid,lhs,txt)VALUES(%d,%d,'", rp->iRule, rp->lhs->index);
    ...
    */
    ...
    for (i = 0; i < lemp->nxstate; i++) {
        for (ap = lemp->sorted[i]->ap; ap; ap = ap->next) {
            if (ap->type == REDUCE || ap->type == SHIFTREDUCE) { ap->x.rp->doesReduce = 1; }
        }
    }
    ...
    */
    ...
    if (lemp->vardest) {
        ...
        for (i = 0; i < lemp->nsymbol; i++) {
            struct symbol* sp = lemp->symbols[i];
            if (sp == 0 || sp->type == TERMINAL || sp->index <= 0 || sp->destructor != 0) continue;
            if (once) {
        ...
        if (dflt_sp != 0) { emit_destructor_code(out, dflt_sp, lemp, &lineno); }
        fprintf(out, "      break;\n");
        lineno++;
    }
    for (i = 0; i < lemp->nsymbol; i++) {
    ...
    */
    ...
    i = 0;
    for (rp = lemp->rule; rp; rp = rp->next) { i += translate_code(lemp, rp); }
    if (i) {
...
void CompressTables(struct lemon* lemp) {
    ...
    for (i = 0; i < lemp->nstate; i++) {
        ...
        for (ap = stp->ap; ap; ap = ap->next) {
            if (ap->type == SHIFT && ap->sp == lemp->wildcard) { usesWildcard = 1; }
...
** token actions.
*/
static int stateResortCompare(void const* a, void const* b) {
    ...
    if (n == 0) {
        ...
        if (n == 0) { n = pB->statenum - pA->statenum; }
    }
    assert(n != 0);
    return n;
}
...
void ResortStates(struct lemon* lemp) {
    ...
    for (i = 0; i < lemp->nstate; i++) { lemp->sorted[i]->statenum = i; }
    lemp->nxstate = lemp->nstate;
    while (lemp->nxstate > 1 && lemp->sorted[lemp->nxstate - 1]->autoReduce) { lemp->nxstate--; }
}
...
/* Deallocate a set */
void SetFree(char* s) { lemon_free(s); }
...
PRIVATE unsigned strhash(char const* x) {
    unsigned h = 0;
    while (*x) h = h * 13 + *(x++);
    return h;
}
...
** than one place.
*/
char const* Strsafe(char const* y) {
    char const* z;
    char* cpy;
...
int Strsafe_insert(char const* data) {
    x1node* np;
    unsigned h;
    unsigned ph;
...
** if no such key. */
char const* Strsafe_find(char const* key) {
    unsigned h;
    x1node* np;
...
*/
struct symbol* Symbol_new(char const* x) {
    struct symbol* sp;
...
*/
int Symbolcmpp(void const* _a, void const* _b) {
...
/* Insert a new record into the array.  Return TRUE if successful.
** Prior data with the same key is NOT overwritten */
int Symbol_insert(struct symbol* data, char const* key) {
    x2node* np;
    unsigned h;
    unsigned ph;
...
/* Return a pointer to data assigned to the given key.  Return NULL
** if no such key. */
struct symbol* Symbol_find(char const* key) {
    unsigned h;
    x2node* np;
...
int Configcmp(char const* _a, char const* _b) {
...
int Configtable_insert(struct config* data) {
    ...
    np = x4a->ht[h];
    while (np) {
        if (Configcmp((char const*)np->data, (char const*)data) == 0) {
            /* An existing entry with the same key is found. */
            /* Fail because overwrite is not allows. */
            return 0;
        }
    ...
    if (x4a->count >= x4a->size) {
        ...
        array.tbl = (x4node*)lemon_calloc(arrSize, sizeof(x4node) + sizeof(x4node*));
        if (array.tbl == 0) return 0; /* Fail due to malloc failure */
...
struct config* Configtable_find(struct config* key) {
    ...
    np = x4a->ht[h];
    while (np) {
        if (Configcmp((char const*)np->data, (char const*)key) == 0) break;
        np = np->next;
    }
    return np ? np->data : 0;
}
