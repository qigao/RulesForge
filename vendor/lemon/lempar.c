...
void* ParseAlloc(void* (*mallocProc)(YYMALLOCARGTYPE)ParseCTX_PDECL) {
    ...
    if (yypParser) { ParseCTX_STORE ParseInit(yypParser ParseCTX_PARAM); }
...
void ParseFinalize(void* p) {
    ...
    ** element left in the stack */
    ...
    while (yytos > pParser->yystack) {
        ...
        if (yyTraceFILE) { fprintf(yyTraceFILE, "%sPopping %s\n", yyTracePrompt, yyTokenName[yytos->major]); }
#endif
        if (yytos->major >= YY_MIN_DSTRCTR) { yy_destructor(pParser, yytos->major, &yytos->minor); }
        yytos--;
    }
...
static void yy_shift(yyParser* yypParser,     /* The parser to be shifted */
    ...
    if (yyNewState > YY_MAX_SHIFT) { yyNewState += YY_MIN_REDUCE - YY_MIN_SHIFTREDUCE; }
...
void Parse(void* yyp,             /* The parser */
    ...
    while (1) { /* Exit by "break" */
        ...
        if (yyact >= YY_MIN_REDUCE) {
            ...
            if (yyTraceFILE) {
                ...
                if (yysize) {
                    fprintf(yyTraceFILE, "%sReduce %d [%s]%s, pop back to state %d.\n", yyTracePrompt, yyruleno,
                            yyRuleName[yyruleno], yyruleno < YYNRULE_WITH_ACTION ? "" : " without external action",
            ...
            **
            */
            if (yypParser->yyerrcnt < 0) { yy_syntax_error(yypParser, yymajor, yyminor); }
