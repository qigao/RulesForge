...
- `from jmespath(...)` for JSON extraction
...
- DSL syntax is unchanged: continue using `from jmespath(...)`, `from dsv(...)`, `from csv(...)`.
...
```rfl
$p: Purchase() from jmespath("{\"orders\":[{\"amount\":120.5}]}", "orders[*]")
```
...
when
    $t: Trigger()
    $p: Purchase(amount > 100.0) from jmespath(file("data/orders.json"), "orders[*]")
then
    insert Matched { }
end
```
...
`accumulate(...)` source pattern can also use `from jmespath(...)` / `from dsv(...)` / `from csv(...)`.
