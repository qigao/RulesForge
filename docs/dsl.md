...
**新功能**: JMESPath JSON 查询支持已在 JavaScript 上下文中全面可用，通过 `jmespath()` 函数，实现强大的 JSON 数据处理和转换。
...
- **Bound Variables:** Variables bound in the `when` block (e.g., `$c`) are available in JavaScript without the ` # Drools-Inspired Rule Language (DRL) Grammar
...
- **`from` Clause:** Modifies the data source for a pattern.
 ...
 prefix (e.g., `c`). You can access their fields like `c.name`.
