...
- **Conditional Elements:**
  ...
  - `eval( <lua_expression> )`: Executes a boolean Lua expression in the context of the current match.
...
The Right-Hand Side (RHS) contains the actions to be executed when the rule fires. The RHS is **Lua code**.

- **Bound Variables:** Variables bound in the `when` block (e.g., `$c`) are available in Lua without the `$` prefix (e.g., `c`). You can access their fields like `c.name`.
...
eval-pattern = "eval" OWS "(" *any-char ")" ; content is opaque Lua
...
; --- RHS (then block) ---
rhs = *any-char ; content is opaque Lua code until 'end'