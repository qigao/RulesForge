...
想象一下，你需要从一个深层嵌套的 JSON 对象中提取特定信息，或者从一个数组中筛选出符合条件的元素，再或者对某个字段进行聚合计算。如果没有 JMESPath，你可能需要：

-   **冗长的点语法链**: `data.users[0].profile.address.city`
-   **复杂的循环和条件判断**: 遍历数组，使用 `if` 语句进行过滤。
-   **手动聚合**: 编写循环来计算 `sum`、`max`、`min` 等。

而 JMESPath 允许你用简洁、声明式的表达式完成这些任务，大大减少了代码量，提高了可读性，并降低了出错的概率。它就像是 JSON 数据的“SQL”，让你能够以更直观的方式与数据交互。

## 当前支持的功能

### ✅ 已实现的 JMESPath 功能 (基于 jsoncons)

现在，Drills 支持 JMESPath 规范的绝大部分功能，包括：
...
现在，当 JMESPath 查询遇到无效 JSON 输入或语法错误时，会抛出 JavaScript 异常。你可以使用 `try-catch` 块来捕获这些错误。
...
1.  **优先使用 JMESPath 表达式**: 尽可能将数据提取和转换逻辑封装在 JMESPath 表达式中，而不是在 JavaScript 中手动编写循环和条件。这会使你的规则更简洁、更易读。
2.  **理解数据结构**: 在编写 JMESPath 表达式之前，确保你对 JSON 数据的结构有清晰的理解。
3.  **测试你的表达式**: 对于复杂的 JMESPath 表达式，建议在独立的工具（如在线 JMESPath 调试器）中进行测试，以确保它们按预期工作。
4.  **处理 `null` 结果**: JMESPath 查询在找不到匹配项时通常返回 `null`。在你的 JavaScript 代码中，始终检查 `jmespath()` 函数的返回值是否为 `null` 或 `undefined`。
