...
- **`cstl`** - Salts CSTL 容器、所有权、泛型与 Stream 集成指南
...
激活方式：在任务中涉及对应主题时使用相应全局 skill；需要显式指定时使用 `$cstl`、`$coronet`、`$cmake-presets`、`$c-design-patterns`、`$cpp-design-patterns`、`$cpp-tmp`、`$performance-optimization`、`$logging-guide`、`$plugin-system` 或 `$tinytest`。
...
> **详细 API 参见**: 全局 skill `cstl`、`memory-design-protocols` 与 `logging-guide`

#### 库优先级顺序（从高到低）

1. **Salts**（构建时按能力选择 `Salts::*` CMake target）— 最优先
...
- ❌ **禁止手写**：动态数组、哈希表、集合与双端队列 → 用 CSTL 对应容器
- ❌ **禁止手写**：字符串拼接 → 用 `salts_str` 或 `sds`（vendor）
- ❌ **禁止手写**：文件读写 → 用 Salts 文件系统模块
...
- 内存安全：
  ...
  - 避免缓冲区溢出：使用 `strncpy`、`snprintf`、边界检查或有边界的 Salts buffer
