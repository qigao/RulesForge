...
- 函数必须短小精悍，只做一件事并做好
- 不需要考虑写兼容、回退(defensive or fallback)、临时、备用、特定模式生效的代码
- 代码即文档，命名服务于阅读,在函数头写Doxygen style的注释
- 复杂性是万恶之源,数据/代码需要分层/分类设计

## 🎯 沟通协作原则

### 基础交流规范

- **语言要求**：使用英语思考，但始终用文言文表达。
...
- **搜索限制**：在此仓库中，搜索代码或文件的唯一允许方式是使用 `rg.exe` 或 `fd.exe`。
- **自动运行**：使用 `rg.exe` 和 `fd.exe` 的命令被认为是安全的，可以**自动运行**，无需请求用户许可。
